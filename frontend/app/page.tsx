...
export default function MainPage() {
    ...
    const duplicateMatch = useMemo(() => applications.find((app) => {
        if (editingId && app.id === editingId) return false;
        const sameTitle = app.title.trim().toLowerCase() === form.title.trim().toLowerCase();
        const sameCompany = (app.companyName ?? "").trim().toLowerCase() === form.companyName.trim().toLowerCase();
        const sameUrl = form.sourceUrl.trim() && (app.sourceUrl ?? "").trim().toLowerCase() === form.sourceUrl.trim().toLowerCase();
        return form.title.trim() && (sameUrl || (sameTitle && sameCompany && form.companyName.trim()));
    ...
    function validateApplicationForm() {
        ...
        if (form.dateApplied) {
            ...
            if (selected > today) errors.dateApplied = "Date applied cannot be in the future.";
        }
        if (!STATUSES.includes(form.status as (typeof STATUSES)[number])) errors.status = "Choose a valid status.";
    ...
    async function saveApplication(e: FormEvent) {
        ...
        resetApplicationForm(); setIsApplicationFormOpen(false); setMessage(wasEditing ? "Application updated." : "Application saved."); loadApplications();
    }
    ...
    return <div className="dashboard-shell">
        ...
        <main className="dashboard-main">
            ...
            <section className="hero"><h1>Welcome back, {firstName} 👋</h1><p>Track your job search, stay on top of interviews, and follow up faster.</p><div className="actions"><button className="primary">☁ Import Job</button><button className="secondary" onClick={openCreateApplication}>＋ Add Application</button><button className="secondary">▥ View Pipeline</button></div></section>
            ...
            <section className="panel pipeline-panel"><div className="panel-title"><div><h2>▣ Application Tracker <span>ⓘ</span></h2><p>Track your applications across stages.</p></div><button className="primary" onClick={openCreateApplication}>＋ Add Application</button></div><div className="filter-row" aria-label="Application filters"><select value={filters.status} onChange={(e) => setFilters({ ...filters, status: e.target.value })}><option value="">All statuses</option>{STATUSES.map((s) => <option key={s} value={s}>{STATUS_LABELS[s]}</option>)}</select><select value={filters.source} onChange={(e) => setFilters({ ...filters, source: e.target.value })}><option value="">All sources</option>{SOURCES.map((source) => <option key={source}>{source}</option>)}</select><input placeholder="Company" value={filters.company} onChange={(e) => setFilters({ ...filters, company: e.target.value })} /><input type="date" aria-label="Start date" value={filters.startDate} onChange={(e) => setFilters({ ...filters, startDate: e.target.value })} /><input type="date" aria-label="End date" value={filters.endDate} onChange={(e) => setFilters({ ...filters, endDate: e.target.value })} /><button onClick={() => loadApplications()}>Apply filters</button></div><div className="kanban">{DASHBOARD_STATUSES.map((status) => <section key={status} className={`lane ${status.toLowerCase()}`} onDragOver={(e) => e.preventDefault()} onDrop={(e) => { const id = e.dataTransfer.getData("text/plain"); if (id) transitionStatus(id, status); }}><h3>{STATUS_LABELS[status]}</h3><strong>{grouped[status].length}</strong><div className="dropzone">{grouped[status].map((a) => <article key={a.id} className="job-card" draggable onDragStart={(e) => e.dataTransfer.setData("text/plain", a.id)}><b>{a.title}</b><span>{a.companyName ?? "Unknown"}</span><small>{a.source ?? "No source"}</small><div><button onClick={() => startEdit(a)}>Edit</button><button onClick={() => removeApplication(a.id)}>Delete</button><button onClick={async () => { const next = openTimelineId === a.id ? null : a.id; setOpenTimelineId(next); if (next && !historyByApp[a.id]) await loadHistory(a.id); }}>History</button></div>{openTimelineId === a.id && <ul>{(historyByApp[a.id] ?? []).map((entry) => <li key={entry.id}>{entry.message}</li>)}</ul>}</article>)}</div></section>)}</div>{applications.length === 0 && <div className="empty-tracker"><div>▰</div><h3>Your pipeline is empty</h3><p>Import jobs or add applications to start tracking.</p><button className="secondary" onClick={openCreateApplication}>＋ Add Application</button></div>}</section>
            ...
            {isApplicationFormOpen && <div className="drawer-backdrop" onClick={closeApplicationForm}><aside className="application-drawer" role="dialog" aria-modal="true" aria-labelledby="application-drawer-title" onClick={(e) => e.stopPropagation()}><form onSubmit={saveApplication}><header className="drawer-header"><button type="button" className="drawer-close" onClick={closeApplicationForm}>←</button><div><h2 id="application-drawer-title">{isEditing ? "Edit application" : "Add application"}</h2><p>{isEditing ? "Update role details without losing your place in the pipeline." : "Track a role in your search and keep next steps organized."}</p></div></header>{duplicateMatch && <p className="duplicate-warning">Possible duplicate: {duplicateMatch.title} at {duplicateMatch.companyName ?? "Unknown company"}.</p>}<section className="form-section"><h3>Primary details</h3><label>Job title *<input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. Senior Frontend Engineer" required /></label>{formErrors.title && <span className="field-error">{formErrors.title}</span>}<label>Company<input value={form.companyName} onChange={(e) => setForm({ ...form, companyName: e.target.value })} placeholder="e.g. Stripe" /></label><label>Status<select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}>{STATUSES.map((s) => <option key={s} value={s}>{STATUS_LABELS[s]}</option>)}</select></label>{formErrors.status && <span className="field-error">{formErrors.status}</span>}<label>Location<input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} placeholder="e.g. Remote, New York, NY" /></label><label>Date applied<input type="date" value={form.dateApplied} onChange={(e) => setForm({ ...form, dateApplied: e.target.value })} /></label>{formErrors.dateApplied && <span className="field-error">{formErrors.dateApplied}</span>}</section><section className="form-section"><h3>Source details</h3><label>Source<select value={form.source} onChange={(e) => setForm({ ...form, source: e.target.value })}>{SOURCE_OPTIONS.map((source) => <option key={source || "blank"} value={source}>{source || "Select a source"}</option>)}</select></label><label>Job URL<input type="url" value={form.sourceUrl} onChange={(e) => setForm({ ...form, sourceUrl: e.target.value })} placeholder="https://..." /></label>{formErrors.sourceUrl && <span className="field-error">{formErrors.sourceUrl}</span>}</section><section className="form-section"><h3>Notes</h3><label>Notes<textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Paste notes, recruiter messages, or next steps..." rows={6} /></label></section><footer className="drawer-footer">{isEditing && editingId && <button type="button" className="danger" onClick={() => removeApplication(editingId)}>Delete application</button>}<button type="button" className="secondary" onClick={closeApplicationForm}>Cancel</button><button className="primary">{isEditing ? "Update application" : "Save application"}</button></footer></form></aside></div>}
        </main>
    </div>;
}
